...
credential_key = 'VMfdBrHSy8QpZZMWx04DDPbFK'
        ...
        for page_num in range(0, total_pages + 1):
            ...
            if user_response.ok:
                ...
                for item in user_json['data']:
                    ...
                    # If the submission is an album, get the album and loop through it
                ...
                error_tries += 1
                if error_tries < 5:
                    # After waiting 1 minute, try to download the user submissions again
                    time.sleep(60)
                    download_user_submissions(username, error_tries)
                else:
...
def handle_command():
    ...
    if website != credentials.website:
        return False
    users = ['bsquidwrd', 'TheSmartyCat', 'ropebaby', 'MaxDragon']
    for user in users:
